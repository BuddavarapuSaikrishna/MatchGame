...
const Home = () => {
  ...
  const [activeImg, SetActiveImg] = useState(imagesList[0].imageUrl)
  ...
  const ChangeTabId = id => {
    const currentTab = tabsList.find(eachItem => eachItem.tabId === id)
    SetActiveTabId(currentTab.tabId)
  }

  const ChangeImage = image => {
    if (activeImg === image) {
      const randomImage = Math.floor(Math.random() * imagesList.length)
      SetActiveImg(imagesList[randomImage].imageUrl)
      SetCurrentScore(prevState => prevState + 1)
    } else {
      const NotMatch = activeImg === image
      SetNoMatch(!NotMatch)
    }
  }
  ...
  const ResetData = () => {
    ...
    SetTimer(60)
    SetNoMatch(false)
    SetActiveImg(imagesList[0].imageUrl)
  }

  return (
    <AppContainer>
      {timer === 0 || noMatch ? (
        <GameOver Score={currentScore} Reset={ResetData} />
      ) : (
        <>
          <Header Score={currentScore} Timer={timer} />
          <ItemDisplayContainer src={activeImg} alt="match" />
          ...
          <TabsListContainer>
            {tabsList.map(eachItem => (
              <TabItems
                TabDetails={eachItem}
                key={eachItem.tabId}
                GetTabId={ChangeTabId}
                ActiveTabId={eachItem.tabId === activeTabId}
              />
            ))}
          </TabsListContainer>
          <ImagesListContainer>
